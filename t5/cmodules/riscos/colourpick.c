  ...
  /*reportf("got colourpicker window_handle " UINTPTR_XTFMT, p_colourpicker_callback->window_handle);*/
    ...
    case COLOUR_PICKER_NORMAL:
        ...
        if(HOST_WND_NONE != p_colourpicker_callback->parent_window_handle)
            if(!winx_register_child_window(p_colourpicker_callback->parent_window_handle, p_colourpicker_callback->window_handle, FALSE)) /* send opens via queue */
                assert0();
        break;
    ...
    _In_z_      PC_U8Z p_title)
{
  ...
  /*colourpicker_callback.chosen = FALSE; already FALSE */
...
#if 0 /* <<< not yet */
    else if(event_query_submenudata_valid())
    {
        GDI_POINT tl;
        status_assert(event_read_submenudata(NULL, &tl.x, &tl.y)); /* ask about menu position */

colour_picker_block.tl_x = tl.x;
colour_picker_block.tl_y = tl.y;
    }
#endif
