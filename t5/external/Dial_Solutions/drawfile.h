...
/* SKS: ON WIN16? use short ints like we see from 32-bit Oak Draw Window font list object */
typedef struct
{
 short int      lfHeight;
 short int      lfWidth;
...
typedef struct {
                ...
                U32     length; /* SKS was long (signed) */
               } draw_diag;
...
---*/
long FAR PASCAL drawfile_round(double r); /* SKS renamed to avoid C99 collision - it is still exported as round() */
